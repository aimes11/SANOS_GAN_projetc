...
from bs import BSInputs, price, forward, discount, vega
...
def implied_vol( mkt_price: float, inp: BSInputs, opt_type: OptionType = "call", vol_low: float = 1e-6, vol_high: float = 5.0, tol: float = 1e-8, max_iter: int = 200,) -> Optional[float]:
    ...
    for _ in range(max_iter):
        tmp = BSInputs(S, K, T, r, q, sigma)
        fs = price(tmp, opt_type) - mkt_price
        ...
        if v < 1e-12:
            ...
            sigma_new = sigma - fs / v
            if not (a < sigma_new < b):
                sigma_new = 0.5 * (a + b)
